...
class HoaHongBacSi extends Component {
    constructor (props) {
    super(props)
    this.state = {
        khammoi: [],
...
componentDidMount(){
    ...
    axios.get('/index.php/api/getallkhammoi').then(response => {
        this.setState({
...
{/* /tabs */}
          ...
          <table className="table-bordered table-hover table color-table primary-table" >
            ...
            {khammoi.map((cd, index) => ( 
              <tr key={cd.id} id={"ketquadieutriitem"+cd.id} data-itemcd={cd.id}>
                ...
                <td data-icd={cd.id} id={"cotdichvu"+cd.id}>{"ST-"+cd.idkhachhang}</td>
                <td> <img src={'./public/uploads/customer/'+cd.anhdaidien}  width='70px' height='70px'/></td>
                <td>{cd.hotenkhachhang}</td>
   ...
   .then(response => {
    ...
    if(response.data[0].tongsaugiam==0)
    {
      document.getElementById("trangthaithanhtoan"+cd.id).innerHTML='<a class="btn btn-danger">Chưa tạo chi phí</a>'
    }
   ...
   .catch(error => {
                ...
                <td><span  id={"dathanhtoan"+cd.id}>0</span> VNĐ</td>  
                <td data-icd={cd.id}>  <span  id={"tongthanhtoan"+cd.id}>0</span> VNĐ
                {(() => {
                      ...
                      .then(response => {
                       ...
                       document.getElementById("dathanhtoan"+cd.id).innerText=response.data[1].dathanhtoan.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")
                       {(() => {
                        if (tongconlai<=0 && Number(response.data[0].phaithanhtoan)!=0) {
                          
                          var hoahong=Number(response.data[0].phaithanhtoan)*0.1
                          document.getElementById("trangthaithanhtoan"+cd.id).innerHTML='<a class="btn btn-success">Đã thanh toán</a>'
                          document.getElementById("hoahong"+cd.id).innerHTML=hoahong.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"
                          tonghoahong=tonghoahong+hoahong;
                          document.getElementById("tonghoahong").innerText=tonghoahong.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"
                        }
                        else{
                          if(Number(response.data[0].phaithanhtoan)!=0)
                          {
                            document.getElementById("trangthaithanhtoan"+cd.id).innerHTML='<a class="btn btn-warning">Chưa thanh toán xong</a>'
                            document.getElementById("hoahong"+cd.id).innerHTML="0 VNĐ"
               ...
               <td id={"trangthaithanhtoan"+cd.id}><a className="btn btn-danger">Chưa tạo chi phí</a></td>
               <td id={"hoahong"+cd.id}>0 VNĐ</td>
