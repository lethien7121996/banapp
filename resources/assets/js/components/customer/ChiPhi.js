...
handleFieldChange (event) {
   ...
   else
   {
     ...
     if(event.target.name == 'soluong')
     {
      ...
      else
      {
        ...
        var mdieutri = document.getElementById("ten").value
 ...
 handleChiTietChiPhi(event)
 {
   ...
   axios.get('/index.php/api/chitietchiphi/'+idpc)
     .then(response => {
       ...
       var selectdv=document.getElementById("updateten").childNodes;
       for(var i = 0; i < selectdv.length; i++) {
         var datadv=selectdv[i].value;
         if(datadv==response.data["ten"])
         {
 ...
 getdulieuxoa(event)
  ...
  render() {
 const { chiphi,dieutridichvulist,chitietthanhtoan,errorgiamgia,tientichluy,thongkechiphi,idkhachhang,usercurrent } = this.state
          ...
          <table className="table-bordered table-hover table color-table primary-table" >
            ...
            {chiphi.map((cd, index) => ( 
              <tr key={cd.id} id={"ketquadieutriitem"+cd.id} data-itemcd={cd.id}>
                ...
                <td data-icd={cd.id}>{cd.giamgia.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")} VNĐ</td>
                ...
                <td className="btnaction">
                  <button data-idchiphi={cd.id} onClick={this.getdulieuxoa} data-toggle="modal" data-target="#xoakhachhang" className="icon-list-demo btn btn-danger btn-circle btn-xl" data-idchiphi={cd.id}>
          ...
          <form onSubmit={this.handleCreateChiPhi}>
              ...
              <div className="form-group">
                    ...
                    <div className="col-md-4">
                        ...
                        <select className="form-control" name="ten" id="ten" onChange={this.handleFieldChange}>
        ...
        <div className="modal-content">
          ...
          <form onSubmit={this.handleUpdateChiPhi}>
              ...
              <div className="form-group">
                    ...
                    <div className="col-md-4">
                        <label htmlFor="recipient-name" className="control-label">Tên dịch vụ điều trị</label>
                        <select className="form-control" name="ten" id="updateten" onChange={this.handleFieldChange}>
                            <option value="chưa chọn">Chọn dịch vụ điều trị</option>
                            {dieutridichvulist.map(cd => ( 
                            <option id={"itemdieutridichvu"+cd.id} key={cd.id}  value={cd.id} >{cd.ten}</option>
                        ))}
...
<div className="modal-dialog" role="document">
  <div className="modal-content">
    ...
    <button type="button" className="btn btn-default" data-dismiss="modal" id="btndongdulieu">Đóng</button>
                                            <button id="btnxoadulieu" onClick={this.handleDeleteChiPhi} type="button" className="btn btn-primary">Xóa</button>
    </div>
