...
handleFieldChange (event) {
 ...
 getdulieuxoa(event)
  ...
  render() {
 const { thanhtoan,chiphi,thongkechiphi,tientichluy,chitietthanhtoan,tongphaitra,tongdatra,tongconlai,usercurrent } = this.state
            ...
            {thanhtoan.map((cd, index) => ( 
              <tr key={cd.id} id={"ketquadieutriitem"+cd.id} data-itemcd={cd.id}>
               ...
               {(() => {
                         ...
                         {
                             return (
                              <button data-idthanhtoan={cd.id} className="icon-list-demo btn btn-danger btn-circle btn-xl" data-idthanhtoan={cd.id}>
                              <Link to={"/hoa-don/"+this.state.idkhachhang+"/"+this.state.idkhammoi+"/"+cd.id}><i className="ti-printer"></i></Link>
                              </button>
                             )
                         }
                         else
          ...
          <form onSubmit={this.handleCreateThanhToan}>
            ...
            <tbody>
                ...
                <select className="form-control" name="hinhthucthanhtoan" id="hinhthucthanhtoan" onChange={this.handleFieldChange}>
                            ...
                            <option value={3} >Chuyển khoản và tiền mặt</option>
                    </select>
