...
class DashBoardMonth extends Component {
  ...
  axios.get('/index.php/api/laytienkhtongtheothang/'+month).then(response => {
    this.setState({
  ...
  handleReset(event)
  {
    document.getElementById("bolocdoanhthu").reset()
    axios.get('/index.php/api/laytienkhtongtheothang/'+month).then(response => {
      this.setState({
        khachhanglist: response.data
      })
...
createImageAnhDaiDien(file){
 ...
 reader.readAsDataURL(file);
  ...
  handleBoLocKhachHang(event)
  {
    ...
    console.log(boloc);
    axios.post('/index.php/api/bolocdoanhthukhtheothang/'+month, boloc, headers)
    .then(responses => {
      var tongthanhtoan=0;
      var tongtienconno=0;
      var tongtiendatra=0;
...
const { bacsilist } = this.state;
            ...
            <div className="white-box">
              <div className="r-icon-stats"> <i className="ti-user bg-megna" />
                <div className="bodystate">
                  <h4>{tongkhachhang.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")}</h4> <span className="text-muted">Khách hàng trong tháng</span> </div>
              </div>
            </div>
          </div>
          <div className="col-md-3 col-sm-6">
            <div className="white-box">
              <div className="r-icon-stats"> <i className="ti-shopping-cart bg-info" />
                <div className="bodystate">
                  <h4>{tongdoanhthu.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</h4> <span className="text-muted">Tổng doanh thu trong tháng</span> </div>
              </div>
            </div>
          </div>
          <div className="col-md-3 col-sm-6">
            <div className="white-box">
              <div className="r-icon-stats"> <i className="ti-wallet bg-success" />
                <div className="bodystate">
                  <h4>{tongno.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</h4> <span className="text-muted">Tiền nợ trong tháng</span> </div>
              </div>
            </div>
          </div>
          <div className="col-md-3 col-sm-6">
            <div className="white-box">
              <div className="r-icon-stats"> <i className="ti-wallet bg-inverse" />
                <div className="bodystate">
                  <h4>{tongthucnhan.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</h4> <span className="text-muted">Doanh thu thực trong tháng</span> </div>
              </div>
            </div>
          </div>
        </div>
        <div className="row el-element-overlay">
        <form onSubmit={this.handleBoLocKhachHang} id="bolocdoanhthu" className="row col-md-12 m-b-40">
  ...
  <div className="col-lg-6">
              ...
              <input type="text" className="form-control doanhthudatepicker"  name="ngaythanhtoan" autocomplete="off" placeholder="dd/mm/yyyy" onChange={this.handleFieldChange} />
              <span className="input-group-addon iccale"><i className="icon-calender" /></span>
           
              </div>
              <div className="input-group hidden" id="khoanngaydatao">
            
              <input className="form-control input-daterange-datepicker-month" type="text" autocomplete="off" name="khoangngaythanhtoan" value="" placeholder="dd/mm/yyyy - dd/mm/yyyy" onChange={this.handleFieldChange} />
              <span className="input-group-addon iccale"><i className="icon-calender" /></span>
        ...
        <div className="col-lg-6">
              ...
              <input type="text" className="form-control" name="chuoicantim" autocomplete="off" placeholder="Họ tên" onChange={this.handleFieldChange} />
             
              </div>
              </div>
              <div className="col-lg-4">
              <div className="input-group">
              <input type="submit" id="buttimkiem" className="btnfullwidth btn btn-primary" value="Tìm kiếm"/>
        ...
        <div className="row">
                    <div className="col-lg-12">
                        <div className="white-box">
                            <h3 className="box-title m-b-0">Danh sách khách hàng thanh toán theo tháng {month}<button className=" m-l-20 btn btn-primary" onClick={this.handleReset}><i className="fa fa-refresh"></i></button></h3>
                            ...
                            <table className="table-bordered table-hover table color-table primary-table">
                                ...
                                <tfoot style={{background: '#e4e7ea'}}>
              <tr>
              <th  style={{    fontWeight: 'bolder',fontSize: '20px'}}>MSKH</th>
                                        <th  style={{    fontWeight: 'bolder',fontSize: '20px'}}></th>
                                        <th  style={{    fontWeight: 'bolder',fontSize: '20px'}}>Tổng</th>
                                        <th id="tongchiphi" style={{    fontWeight: 'bolder',fontSize: '20px'}}></th>
                                        <th id="tiendathanhtoan"  style={{    fontWeight: 'bolder',fontSize: '20px'}}></th>
                                        <th id="tienconno"  style={{    fontWeight: 'bolder',fontSize: '20px'}}></th>
                                ...
                                {khachhanglist.map((dv, index)  => (
                                    <tr key={dv.ID}>
                                        <td className="title"><a href="#" onClick={e => e.preventDefault()}>{"ST-"+dv.ID}</a></td>
                                        ...
                                        <p id={"tienthanhtoan"+dv.ID}>{dv.tongsaugiam.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</p>                 
                                        </td>
                                        <td>
                                        {(() => {
                    
                        if(typeof listsotienkh[index] === "string")
                        {
                          return(
                            <p id={"tiendathanhtoan"+dv.ID}>{listsotienkh[index].replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</p>   
                          )
                        ...
                        else
                        {
                          return(
                              ...
                              <p id={"tiendathanhtoan"+dv.ID}>{listsotienkh[index]+" VNĐ"}</p>   
 ...
 {(() => {
                        var sotienconlai=Number(dv.tongsaugiam)-Number(listsotienkh[index])
return(
  <p id={"tienno"+dv.ID}>{sotienconlai.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</p>  
)
                    })()}
                                </td>
                        ...
                        {
                          return(
                            <button type="button" className="btn btn-block  btn-success">Đã thanh toán xong</button>
                          )
                        ...
                        {
                          return(
                            <button type="button" className="btn btn-block  btn-danger">Thanh toán thừa cho khách</button>
                          )
                        }
                        else
                        {
                          return(
                            <button type="button" className="btn btn-block  btn-warning">Chưa thanh toán xong</button>
                          )
                        }
                    })()}
